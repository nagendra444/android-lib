...
import com.mediamelon.qubit.MMQFPresentationVideoTrackInfo;
...
public class MMSmartStreaming implements MMQFQubitizer.onInitializationCompleteListener {
  ...
  public static void registerMMSmartStreaming(String playerName, String customerID, String componentName, String subscriberID, String domainName){
    registerMMSmartStreaming(playerName,customerID,componentName,subscriberID,domainName);
  }
  ...
  public static void registerMMSmartStreaming(String playerName, String customerID, String componentName, String subscriberID, String domainName, String subscriberType){
    registerMMSmartStreaming(playerName,customerID,componentName,subscriberID,domainName,subscriberType);
  }

    public static void registerMMSmartStreaming(String playerName, String customerID, String componentName, String subscriberID, String domainName, String subscriberType, String subscriberTag,boolean doHash){
    try{
      if(logSTrace){
      ...
      SDKExperienceProbe.getInstance().registerSDKUserInfo(playerName, customerID, componentName, subscriberID, domainName, subscriberType,subscriberTag,doHash);
    }
    catch(Exception e){
      e.printStackTrace();
    }
  ...
  */
   ...
   *                     If not known, set it to -1.
   */
  public static void reportDeviceInfo(String brand, String deviceModel, String os, String osVersion, String telecomOperator, Integer screenWidth, Integer screenHeight, String deviceMarketingName){
    try{
      if(logSTrace){
      ...
      SDKExperienceProbe.getInstance().setDeviceInformation(brand, deviceModel, os, osVersion, telecomOperator, screenWidth, screenHeight, deviceMarketingName);
    }
    catch(Exception e){
      e.printStackTrace();
    }
  ...
  public static void reportAppInfo(String appName, String appVersion){
      try{
          if(logSTrace){
              Log.v(StackTraceLogTag, "reportAppInfo - appName=" + appName + ", appVersion=" + appVersion);
          }
          SDKExperienceProbe.getInstance().setAppInformation(appName,appVersion);
      }
      catch(Exception e){
          e.printStackTrace();
      }
  ...
  public static void reportTrackInfo(String audioTrack, String subtitleTrack, boolean isSubtitleActive, boolean isVDSActive, String videoTrack){
      try{
          if(logSTrace){
              Log.v(StackTraceLogTag,"audioTrack=" + audioTrack);
          }
          SDKExperienceProbe.getInstance().setTrackInfo(audioTrack, subtitleTrack, isSubtitleActive, isVDSActive, videoTrack);
      }
      catch(Exception e){
          e.printStackTrace();
      }
  ...
  public static void reportVideoQuality(String videoQuality){
      try{
          if(logSTrace){
              Log.v(StackTraceLogTag,"videoQuality=" + videoQuality);
          }
          SDKExperienceProbe.getInstance().setVideoQuality(videoQuality);
      }
      catch (Exception e){
          e.printStackTrace();
      }
   ...
   *
   */
  public Integer initializeSession(MMQBRMode mode, String manifestURL, String metaURL, String assetID, String assetName, MMSmartStreamingObserver observer){
    return initializeSession(mode,manifestURL,metaURL,assetID,assetName,null,observer);
  }
  public Integer initializeSession(MMQBRMode mode, String manifestURL, String metaURL, String assetID, String assetName, String videoId, MMSmartStreamingObserver observer){
      Integer ret = -1;
    try{
      engineStarted = false;
...
//      else if (mode == MMQBRModereportAdInfo.QBRModeDisabled){
...
//          JSONObject j = cm.getJSONObject();
  ...
  public void reportChunkRequest(MMChunkInformation chunkInfo){
    try{
      if(logSTrace){
        Log.v(StackTraceLogTag, "reportChunkRequest - chunkInfo=" + chunkInfo);
      }
...
//      getVideoTrackForBitrate
      ...
      if(chunkInfo.bitrate > 0 || chunkInfo.trackIdx >= 0){
          ...
          if(repInfo != null){
              ...
              String[] codecs = codecInfo.split(",");
              checkForAcodec(codecs[0],codecs[1]);
              checkForVcodec(codecs[0],codecs[1]);
             // Log.i("PPPP",chunkInfo.bitrate.toString() + "    " + codecInfo );
          }
      }
      SDKExperienceProbe.getInstance().trackSegment(chunkInfo.startTime, size, chunkInfo.bitrate, height, width, -1, aCodec,vCodec);
    }
    catch(Exception e){
      e.printStackTrace();
    }
  ...
  public void reportDownloadRate(Long downloadRate){
    try{
      if(logSTrace){
        Log.v(StackTraceLogTag, "reportDownloadRate - downloadRate=" + downloadRate.toString());
      }
      if (!engineStarted){
  ...
  public void reportPlaybackPosition(Long playbackPos){
    try{
      if(logSTrace){
        Log.v(StackTraceLogTag, "reportPlaybackPosition - playbackPos=" + playbackPos.toString());
      }
      if (!engineStarted){
  ...
  public void reportABRSwitch(Integer prevBitrate, Integer newBitrate){
    try{
      ...
      checkForCodecChange(newBitrate);
    }
    catch(Exception e){
      e.printStackTrace();
    }
  ...
  //checking for codecs had changed for new bitrate

  public void checkForCodecChange(int bitrate){

    MMRepresentation rep = MMQFQubitizer.getInstance().getRepresentationInfo(bitrate,-1);
    String codecInfo = null;

    if(rep!=null){
      codecInfo = rep.codecId();
    }
    try {
      String[] result = codecInfo.split(",");
      String codec1 = result[0];
      String codec2 = result[1];

      checkForAcodec(codec1, codec2);
      checkForVcodec(codec1, codec2);

      if (prevAcodec == null) {
        SDKExperienceProbe.getInstance().reportACodecShift(prevAcodec, aCodec);
      }
      if (prevVcodec == null) {
        SDKExperienceProbe.getInstance().reportVCodecShift(prevVcodec, vCodec);
      }

      if (prevAcodec != null && !prevAcodec.equals(aCodec)) {
        SDKExperienceProbe.getInstance().reportACodecShift(prevAcodec, aCodec);
      }
      if (prevVcodec != null && !prevVcodec.equals(vCodec)) {
        SDKExperienceProbe.getInstance().reportVCodecShift(prevVcodec, vCodec);
      }
      prevAcodec = aCodec;
      prevVcodec = vCodec;
  ...
  public void checkForAcodec(String codec1,String codec2){

    String[] aCodecs = {"ac-3", "alac", "ec-3", "fLaC", "mp4a"};

    for(String str:aCodecs){
      if(codec1.contains(str)){
        aCodec = codec1;
        break;
      }
      if(codec2.contains(str)){
        aCodec = codec2;
        break;
      }
    }
  ...
  public void checkForVcodec(String codec1,String codec2){
    String[] vCodecs = {"avc1", "avc3", "dvh1", "dvhe", "hev1", "hvc1"};
    for(String str:vCodecs){
      if(codec1.contains(str)){
        vCodec = codec1;
        break;
      }
  ...
  public void reportAppError(long errorCode,String errorMessage){
    try{
      if(logSTrace){
        Log.v(StackTraceLogTag, "reportError - errorCode=" + errorCode + " errorMessage=" + ((errorMessage != null) ? errorMessage:""));
      }
      if (!engineStarted){
        Log.v(StackTraceLogTag, "reportError - Engine not initialized properly returning");
        return;
      }
      SDKExperienceProbe.getInstance().onAppError(errorCode,errorMessage);
    }
    catch(Exception e){
      e.printStackTrace();
    }
  ...
  try{
    ...
    SDKExperienceProbe.getInstance().reportAdInfo(adClient, adId, adDuration, adPosition, adType, adCreativeType, adServer, adResolution, adPodIndex, adPositionInPod, adPodLength, isBumper, adScheduledTime, adCreativeId, adUrl, adTitle, adBitrate,timestamp);
  }
    catch(Exception e){
      e.printStackTrace();
    }
      ...
      if(adInfo != null) {
        ...
        SDKExperienceProbe.getInstance()
            .reportAdInfo(adInfo.adClient, adInfo.adID, adInfo.adDuration, adInfo.adPosition,
                ...
                adInfo.adScheduledTime,adInfo.adCreativeId,adInfo.adUrl,adInfo.adTitle,adInfo.adBitrate,adInfo.timestamp);
      }
    }
    catch(Exception e){
      e.printStackTrace();
