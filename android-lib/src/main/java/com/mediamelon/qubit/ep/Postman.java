...
public class Postman implements Runnable {
    ...
    static String hdrCap = null;
    static String widevine = null;
    static String drmVersion = null;
...
//            MMLogger.v(TAG, " POSTMAN isStatsRecord " + record.timestamp);
    ...
    private void setPropertiesForStart(RecordStructure record){
        record.qubitData.get(0).clientInfo.HdrCapbilities = hdrCap;
        record.qubitData.get(0).clientInfo.widevine = widevine;
        record.qubitData.get(0).clientInfo.drmVersion = drmVersion;
        record.qubitData.get(0).clientInfo.systemId = systemId;
    }
    ...
    public void decorateRecord(RecordStructure record){
        ...
        if(isEventRecord(record)) {
            ...
            if(record.qubitData.get(0).pbEventInfo.event == "START"){
                setPropertiesForStart(record);
            }
    ...
    StreamInfo CloneStreamInfo(StreamInfo in){
        ...
        clone.audioTrack = in.audioTrack;
        clone.subtitleTrack = in.subtitleTrack;
        clone.isSubtitleActive = in.isSubtitleActive;
        clone.isVDSActive = in.isVDSActive;
        clone.videoTrack = in.videoTrack;
    ...
    PlaybackInfo ClonePBInfo(PlaybackInfo in){
        ...
        clone.progressMark = in.progressMark;
        return clone;
    }
    ...
    public ClientInfo CloneClientInfo(ClientInfo in){
        ...
        clone.appName = in.appName;
        clone.appVersion = in.appVersion;
        clone.deviceMarketingName = in.deviceMarketingName;
        clone.videoQuality = in.videoQuality;
        clone.HdrCapbilities = in.HdrCapbilities;
        return clone;
    }
