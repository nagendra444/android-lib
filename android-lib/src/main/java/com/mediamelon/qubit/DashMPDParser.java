...
//
    ...
    Vector<MMQFPresentationVideoTrackInfo> videoTrackInfos = new Vector<>();
    public void CreateQubitPresentationInfo(){
        ...
        {
            String aCodec  = null;
            int noOfAdaptationSets1 = presentationDescription_.periods.get(0).adaptationSets.size();
            if(noOfAdaptationSets1>0) {
                ArrayList<AdaptationSet> videoAdaptationSet = new ArrayList<AdaptationSet>();
                for (int j = 0; j < noOfAdaptationSets1; j++) {
//
                    if (presentationDescription_.periods.get(0).adaptationSets.get(j).type
                        == C.TRACK_TYPE_AUDIO) {
                        aCodec = presentationDescription_.periods.get(0).adaptationSets.get(j).representations.get(0).format.codecs;
                    }
                }
            }


            for(int i=0;i<presentationDescription_.periods.size();i++){

                int noOfAdaptationSets = presentationDescription_.periods.get(i).adaptationSets.size();//calucating the adaption set for current period
...
//
                        ...
                        //Iterate thru all the representations of the adaptation set
                        int trackCounter = 0;
                        for(int a = 0; a<videoAdaptationSet.size(); a++){
                            for(int r=0; r<videoAdaptationSet.get(a).representations.size(); r++){
                                Representation rep = videoAdaptationSet.get(a).representations.get(r);
                                ...
                                info.codecInfo = rep.format.codecs+","+aCodec;//externally adding aCodec
...
//
                                ...
                                videoTrackInfos.add(info);
                            }
                        }
                }
            }
        }
        MMQFQubitizer.getInstance().setPresentationVideoTracks(videoTrackInfos);
